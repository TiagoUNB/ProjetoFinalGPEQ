...
De forma prática e tangível, a aplicação conjunta de POPs e Poka-Yoke no Tarumã pode ser implementada através da criação de uma "Estação Padrão de Montagem" nas bancadas da cozinha. Por exemplo, a área de finalização da parmegiana teria um POP visual simplificado fixado à altura dos olhos, contendo um fluxograma de passos rápidos (ex.: 1. Acomodar proteína; 2. Acionar 1 dose de molho; 3. Despejar 1 medidor de queijo). O Poka-Yoke materializa-se nos próprios equipamentos: os recipientes medidores teriam marcas de limite bem visíveis (como linhas de cor vibrante) e os dispensadores de molho poderiam ser adaptados com válvulas de pressão (tipo \textit{pump}) que liberam o volume exato a cada acionamento. Assim, durante o pico de atendimento, o funcionário não precisa deduzir quantidades, eliminando os desvios de porcionamento por meio de restrições físicas e instruções imediatas.
